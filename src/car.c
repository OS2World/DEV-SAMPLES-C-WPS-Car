...
/*
 * WATCOM PORT NOTE: DebugBox needs NO local definition - it arrives via
 * the parent-class header chain (wpdataf.h:81 defines it unconditionally).
 * The original idl duplicated it in a passthru, which Watcom correctly
 * rejected as a non-identical redefinition (E1100); that duplication is
 * gone from idl\car.idl.
 */

/*
 * WATCOM PORT NOTE: modern sc's emitc does not generate SOMInitModule
 * (the old toolkit did).  Left undefined, wlink silently resolved the
 * name from somtk.lib and exported a forwarder to somir.dll - whose
 * SOMInitModule initializes somir's classes, not ours.  Define it here,
 * metaclass first, per the working ClrPalet/MBFolder ports.
 */
#pragma linkage(SOMInitModule, system)
void SOMLINK SOMInitModule(integer4 majorVersion, integer4 minorVersion)
{
    M_CarNewClass(majorVersion, minorVersion);
    CarNewClass(majorVersion, minorVersion);
}
...
/*
 * WATCOM PORT NOTE: the original had "#pragma handler (car_TrapTest)"
 * here - an IBM C Set/2 pragma that marks the function for OS/2
 * exception-handler unwinding.  Open Watcom has no equivalent; without
 * it the "Trap D" menu item will genuinely trap instead of demonstrating
 * recovery, which is acceptable for a demo of what a trap looks like.
 */
...
SOM_Scope HMODULE   SOMLINK carM_clsQueryModuleHandle(M_Car *somSelf)
{
 APIRET rc;
    ...
    */
    ...
    if (hmod == NULLHANDLE)
    {
     /* WATCOM PORT NOTE: was "zString" - an IBM-era typedef absent from
      * the toolkit 4.5 SOM headers; SOM's plain "string" (char *) is
      * what _somLocateClassFile returns. */
     string zsPathName;
     somId id;
       /*
